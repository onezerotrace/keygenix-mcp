...
export const importKeyTool = {
    name: "import_key",
    description: "Import an existing key (mnemonic or private key) into Keygenix TEE. " +
...
// ─── Handler ──────────────────────────────────────────────────────────────────
export async function handleImportKey(config, args) {
    ...
    const { publicKey: preparedPubKey } = await apiCall(config, "POST", walletUrl(config, "/keys/prepare_import"), {});
